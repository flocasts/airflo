...
kubectl config use-context gke_engineering-sandbox-228018_us-central1-a_sandbox
kubectl config set-context --current --namespace=ns-andrew-tan
helm install --namespace=ns-andrew-tan "airflow" stable/airflow
```
...
export POD_NAME=$(kubectl get pods --namespace ns-andrew-tan -l "component=web,app=airflow" -o jsonpath="{.items[0].metadata.name}")
kubectl port-forward --namespace ns-andrew-tan $POD_NAME 8080:8080
